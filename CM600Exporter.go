...
type CM600StatusExporter struct {
	StatusExporter
}

// Collect runs our scrape loop returning each Prometheus metric.
func (e *CM600StatusExporter) Collect(ch chan<- prometheus.Metric) {
	e.totalScrapes.Inc()
...
func (e *CM600StatusExporter) Describe(ch chan<- *prometheus.Desc) {
	// Exporter metrics.
